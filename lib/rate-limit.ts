...
export function rateLimit(
  ...
  if (env.REDIS_URL) {
    console.warn(
      "[rate-limit] Synchronous rateLimit called with Redis; falling back to in-memory limiting. Use rateLimitAsync for Redis-backed limits.",
    );
...
export function withRateLimit(
  ...
  return async function (req: NextRequest): Promise<NextResponse> {
    ...
    response.headers.set(
      "X-RateLimit-Limit",
      String(endpointConfig.maxRequests),
    );
