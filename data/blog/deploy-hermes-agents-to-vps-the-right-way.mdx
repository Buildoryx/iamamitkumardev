---
title: "How to Deploy Hermes Agents to a VPS the Right Way: A Production Playbook (₹600/month, 9 Phases, Zero Public Ports)"
publishedAt: "2026-05-26"
summary: "Deploy Hermes Agent (Nous Research) to a VPS in 9 phases — Tailscale-only access, systemd timers, credential pool fallback, Discord gateway. Real configs, real failure logs, ₹600/month total cost. From a live multi-day-uptime production setup."
...
I run **a multi-agent stack** on a single **4GB VPS** for under **₹600/month**, with multi-day uptime, zero public ports, and a credential pool that auto-rotates when one provider rate limits me. This post is the exact playbook — the 9 phases I went through, the configs that broke, and the ones that actually held.
...
## The stack and what it costs (₹ breakdown)

Real numbers from a typical multi-agent setup as of May 2026:

| Line item | Provider | Monthly cost (₹) | Notes |
|---|---|---|---|
| VPS — 4 vCPU / 4GB RAM / 50GB NVMe | Any reputable provider | ~₹400–₹700 | annual prepay on India-POP providers; works for ~10–15 agents comfortably |
| Domain | Namecheap / GoDaddy | ~₹100 (avg) | optional, for Caddy reverse proxy if you expose anything |
| Tailscale | Tailscale | ₹0 | free tier, up to 100 devices |
| Discord bot | Discord | ₹0 | one bot, one channel, one operator |
| LLM credentials | Mixed (see below) | ₹0–₹4,000 | depends on usage; most can run on free tiers + 1–2 paid creds |
| **Total floor** | | **~₹550/mo** | infra only |
| **Total ceiling** | | **~₹4,500/mo** | with paid LLM credentials |

For comparison: hiring **one** junior dev to baby-sit a content/research workflow at ₹35,000/month is **~63x** more expensive than the infra floor. That's the math the framework is built on.

The LLM cost ceiling is intentional. The credential pool I describe in **Phase 8** is what keeps the floor at ₹0–₹2K/mo even at higher throughput.
...
For Indian builders, an India-POP provider gives you ~30ms RTT and rupee-denominated billing — DigitalOcean Bangalore, Linode Mumbai, AWS Mumbai Lightsail, and several local KVM providers all work. Outside India, any provider with full root access is fine — DigitalOcean, Linode, Vultr, Contabo, OVH.
...
Phase 10 (which I'll publish separately) is **observability + backup** — Caddy reverse proxy on Tailscale Funnel, `rclone` to Backblaze B2 for ₹50/mo cold storage, and Grafana for the metrics that matter.
...
Technically yes, practically no. A 1GB RAM VPS will OOM the moment a vector store loads or fact extraction runs. **Minimum 4GB RAM** for a real agent system. For Indian builders, India-POP providers like DigitalOcean Bangalore, Linode Mumbai or AWS Mumbai Lightsail give you a 4GB VPS in the ₹400–700/mo range. Outside India, DigitalOcean's $24 droplet (4GB RAM) is the equivalent.
...
- **Floor:** ~₹550/month (VPS + domain only)
- **Ceiling:** ~₹4,500/month (with 2-3 paid credentials for higher throughput)

Compare to one junior dev at ₹35,000/month doing the same content+research+QA workflow manually. The ROI math is the actual product here.
...
*If this playbook saved you a weekend, [follow me on X](https://x.com/growthperclick) for more candid implementer write-ups — the failures, the configs, and the ₹ math behind running agent systems out of India. Questions? Reply on the X thread for this article and I'll answer in detail. Building in public is more useful when the public actually gets the build.*
